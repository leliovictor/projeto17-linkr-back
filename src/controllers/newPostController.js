        ...
        if (newPost.message){
            ...
            if(hashtags.length > 0){
                for (let i=0; i<hashtags.length; i++){
                    ...
                    const postId = await newPostRepository.getPosts(newPost.message, data.id);
                    const hashtagId = await newPostRepository.getHashtagsByName(hashtags[i]);
                    await newPostRepository.addNewHashtagPost(newPost.url, hashtagId.rows[0].id, postId.rows[0].id);    
                }
            }
        }
        res.sendStatus(201);
